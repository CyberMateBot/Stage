...
func TestUpdateProfileTheme_OK(t *testing.T) {
    ...
    if out.Theme != ucModels.ThemeDark {
        t.Fatalf("expected dark theme, got %s", out.Theme)
    }
    if repo.exists["42"].UITheme != ucModels.ThemeDark {
        t.Fatalf("repo theme not updated")
    }
}

func TestUpdateProfileTheme_Invalid(t *testing.T) {
    repo := &fakeRepoProfile{exists: map[string]repoModels.Profile{}}
    uc := NewUseCase(repo)
    _, err := uc.UpdateProfileTheme(context.Background(), ucModels.UpdateProfileThemeInput{
        TelegramID: "42",
        Theme:      "neon",
    })
