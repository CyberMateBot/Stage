...
| `PG_DBNAME` | string | `cybermate` | Имя БД |
...
Сборка идёт через Docker и `protoc` (скрипт `scripts/proto-gen.sh`): клонируются `googleapis` и `grpc-gateway` в `third_party/` (каталоги в `.gitignore`), затем генерируются `pkg/api/*.pb.go` и `api/service.swagger.json`. Доступ к `buf.build` не нужен.

```bash
make proto.gen
```

После смены `option go_package` или RPC в `api/service.proto` снова выполните `make proto.gen`.
...
**Локально** миграции нужно применить вручную (после `docker compose up -d postgres`):

```powershell
.\scripts\migrate.ps1
```

В production/CI используется Flyway (см. `.gitlab/`).
...
- CyberMate API (profiles, wallets, referrals)
...
🐛 Нашли баг? [Создайте issue](https://github.com/twelvepills-936/tgapp-/issues)

💬 Есть вопросы? [Открывайте discussion](https://github.com/twelvepills-936/tgapp-/discussions)
